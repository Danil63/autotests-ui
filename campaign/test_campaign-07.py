...
@pytest.mark.CompCpm
def test_campaignn_07(page_with_state_comp: Page):
    page = page_with_state_comp


    print("AUTO-TEST CPM + товар")
    ...
    name_input.fill("AUTO-TEST CPM + товар")
    expect(name_input).to_have_value("AUTO-TEST CPM + товар")
    ...
    option_to_select = page.get_by_text("штангель циркуль").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("штангель циркуль")
    ...
    reels_checkbox = page.get_by_role("checkbox", name="Видео").first
    reels_checkbox.click()

    story_checkbox = page.get_by_role("checkbox", name="Shorts").first
    ...
    # прайс тот же
    format_bitton = page.locator('//button[text()="За просмотры"]')
    format_bitton.wait_for(state="visible", timeout=10000)
    format_bitton.click()
    ...
    expect(value_content_format).to_contain_text("Видео")
    expect(value_content_format).to_contain_text("Shorts") 
