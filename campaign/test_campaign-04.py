...
@pytest.mark.CompFixeServiceAuto
def test_campaignn_04(page_with_state_comp: Page):
    page = page_with_state_comp


    print("AUTO-TEST Фикса + услуга C авто")
    ...
    name_input.fill("AUTO-TEST Фикса + услуга C авто")
    expect(name_input).to_have_value("AUTO-TEST Фикса + услуга C авто")
    ...
    option_to_select = page.get_by_text("Ремонт замков на сумках").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("Ремонт замков на сумках")
