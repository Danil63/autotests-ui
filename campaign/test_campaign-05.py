...
@pytest.mark.CompFixAUTO
def test_campaignn_05(page_with_state_comp: Page):
    page = page_with_state_comp


    print("AUTO-TEST Фикса + товар С авто")
    ...
    name_input.fill("AUTO-TEST Фикса + товар С авто")
    expect(name_input).to_have_value("AUTO-TEST Фикса + товар С авто")
    ...
    option_to_select = page.get_by_text("Лом меди").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("Лом меди")
