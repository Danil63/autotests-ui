...
@pytest.mark.CompFix
def test_campaignn_06(page_with_state_comp: Page):
    page = page_with_state_comp
    
    print("AUTO-TEST Фикса + товар БЕЗ авто")
    ...
    name_input.fill("AUTO-TEST Фикса + товар БЕЗ авто")
    expect(name_input).to_have_value("AUTO-TEST Фикса + товар БЕЗ авто")
    ...
    option_to_select = page.get_by_text("медный брусок").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("медный брусок")
    ...
    # прайс тот же
    format_bitton = page.locator('//button[text()="Фиксированная"]')
    ...
    auto_approval = page.locator('//button[@role="switch"]').last
