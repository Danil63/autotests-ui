...
def test_campaignn_03(page_with_state_comp: Page):
    ...
    name_input.fill("AUTO-TEST Фикса + услуга БЕЗ авто")
    expect(name_input).to_have_value("AUTO-TEST Фикса + услуга БЕЗ авто")
    ...
    option_to_select = page.get_by_text("Мойка окон").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("Мойка окон")
