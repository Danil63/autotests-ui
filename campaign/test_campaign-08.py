...
@pytest.mark.CompFixeServiceAutoCPM
def test_campaignn_08(page_with_state_comp: Page):
    page = page_with_state_comp


    print("AUTO-TEST Бартер + товар ДЛЯ ФИЗИКА")
    ...
    name_input.fill("AUTO-TEST Бартер + товар ДЛЯ ФИЗИКА")
    expect(name_input).to_have_value("AUTO-TEST Бартер + товар ДЛЯ ФИЗИКА")
    ...
    option_to_select = page.get_by_text("напильник").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("напильник")
    ...
    # прайс тот же
    price_input = page.locator('//input[@placeholder="Сумма"]')
