...
@pytest.mark.CompBarterAuto
def test_campaignn_02(page_with_state_comp: Page):
    page = page_with_state_comp


    print("AUTO-TEST Бартер С авто")
    ...
    name_input.fill("AUTO-TEST Бартер С авто")
    expect(name_input).to_have_value("AUTO-TEST Бартер С авто")
    ...
    option_to_select = page.get_by_text("заклепки").first
    option_to_select.wait_for(state="visible", timeout=10000)
    option_to_select.click()

    expect(product_dropdown).to_contain_text("заклепки")
    ...
    price_input.fill("16")
    expect(price_input).to_have_value("16 ₽")
