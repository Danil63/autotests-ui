...
def test_product_06(page_with_state: Page):
    ...
    file_input.set_input_files("/Users/mbookpro403gmail.com/Documents/Мишки/Без названия (3).jpeg")
    ...
    name_input.fill("напильник")
    expect(name_input).to_have_value("напильник")

   
    description_input = page.locator('//textarea[@name="description"]')
    description_input.wait_for(state="visible", timeout=10000)
    description_input.fill("Напильник — это ручной слесарный инструмент, предназначенный для обработки и доводки поверхностей различных материалов:  металла, древесины, пластика. Он представляет собой стальной стержень с насечкой и рукояткой для удобного удержания. Рабочая часть имеет разные формы — плоская, круглая, треугольная или полукруглая, что позволяет выполнять как грубую обработку, так и тонкую подгонку деталей. Насечка бывает крупной и мелкой: первая используется для быстрого снятия материала, вторая — для точного шлифования. Напильник незаменим в машиностроении, столярном и ремонтном деле, его применяют для удаления заусенцев, сглаживания кромок и придания изделиям нужной формы. При правильном использовании инструмент обеспечивает высокое качество обработки.")
    expect(description_input).to_have_value("Напильник — это ручной слесарный инструмент, предназначенный для обработки и доводки поверхностей различных материалов:  металла, древесины, пластика. Он представляет собой стальной стержень с насечкой и рукояткой для удобного удержания. Рабочая часть имеет разные формы — плоская, круглая, треугольная или полукруглая, что позволяет выполнять как грубую обработку, так и тонкую подгонку деталей. Насечка бывает крупной и мелкой: первая используется для быстрого снятия материала, вторая — для точного шлифования. Напильник незаменим в машиностроении, столярном и ремонтном деле, его применяют для удаления заусенцев, сглаживания кромок и придания изделиям нужной формы. При правильном использовании инструмент обеспечивает высокое качество обработки.")
    ...
    price_input.fill("7")
    expect(price_input).to_have_value("7")
