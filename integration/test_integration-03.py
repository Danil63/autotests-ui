...
def test_integration_01():
    ...
    with sync_playwright() as p:
        ...
        search_input.fill('Мойка окон')
        expect(search_input).to_have_value('Мойка окон')
...
def test_integration_02():
    with sync_playwright() as p:
        ...
        # Работа с карточками
        # Ждём карточку с текстом (частичное совпадение)
        card_company = page.locator('//*[contains(text(),"Фикса")]').first
        card_company.wait_for(state="visible", timeout=60000)  # ждём до 60 сек
        card_company.scroll_into_view_if_needed()
        card_company.click()
...
def test_integration_03():
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            try:
                card_company_integration = page.locator('//span[text()="Мойка окон"]')
...
def test_integration_05():
    ...
    with sync_playwright() as p:
        ...
        page.locator('//span[text()="Мойка окон"]').click()
...
def test_integration_07():
    ...
    with sync_playwright() as p:
        ...
        card = page.locator('//span[text()="Мойка окон"]').first
        card.click()
