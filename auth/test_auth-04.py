...
def test_auth_04(page):
        ...
        expect(error_alert).to_have_text("Произошла ошибка", timeout=10000)

        error_alert2 = page.locator('//div[@class="text-sm opacity-90"]')
        expect(error_alert2).to_have_text("Пользователь не найден", timeout=10000)
