...
def test_auth_12(page):
        ...
        expect(phone_login_input).to_have_value(re.compile(r"\+7\s*\(\d{3}\)\s*\d{3}-\d{2}-\d{2}"))
        print(f"placeholder с +7 сменился на {login_value}")
